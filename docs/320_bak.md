...
本輸入方案，可透【小狼毫】輸入法提供之圖形介面，經由以下操作步驟`切換【漢字標音選項】`。
...
切換方式：按 [Ctrl] + [`] 鍵，進入【輸入方案】之選項切換功能，變更【漢字標音音選項】。可選項目與 [310_反切輸入法【台語音標】設計規格.md](./310_反切輸入法【台語音標】設計規格.md) 相同。
...
完成操作情境之【步驟7】後，此時之【輸入編輯列】與【候選字清單】顯示，大致如下所示：
...
local config = {
    ...
    schema_id = {
        -- 反切輸入法類型
        huan_ciat_tlpa = "tlpa",  -- 預設：台語音標（tlpa）
        huan_ciat_tps = "tlpa",  -- 預設：方音符號（tps）
        -- 注音輸入法類型
        zu_im_bpm2 = "bpm2",  -- 預設：台語注音二式（bpm2）
        zu_im_tlpa = "tlpa_bpm",  -- 預設：台語音標之注音符號（tlpa_bpm）
        zu_im_tps = "tps",  -- 預設：方音符號（tps）
        -- 拼音輸入法類型
        phing_im_bp = "bp",  -- 預設：閩拼方案（bp）
        phing_im_bpm2 = "bpm2",  -- 預設：台語注音二式（bpm2）
        phing_im_poj = "poj",  -- 預設：白話字（poj）
        phing_im_tl = "tl",  -- 預設：台羅拼音（tl）
        phing_im_tlpa = "tlpa",  -- 預設：台語音標（tlpa）
    },
    -- 漢字標音用符號
