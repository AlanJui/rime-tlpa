...
import os
import sys
from ctypes import POINTER, Structure, c_char_p, c_int, c_uint64, c_void_p, sizeof
...
def dump(rime, sid, label):
    ...
    if not rime.RimeGetContext(sid, ctypes.byref(ctx)):
        print(f"!! {label} no context")
        return
    preedit = (ctx.composition.preedit or b"").decode("utf-8", "replace")
    print(f"\n== {label}\npreedit=[{preedit}] n={ctx.menu.num_candidates}")
...
def main():
    ...
    for keys in ["2,r5", "2,r", "2,5"]:
        rime.RimeClearComposition(sid)
        ok = rime.RimeSimulateKeySequence(sid, keys.encode())
        dump(rime, sid, f"{keys!r} ok={ok}")
